...
public class ObjectQueueExecutor implements Runnable {
			...
			if( qaResult.size() == 0 ) {
				...
				*/			
				...
				try 
				{
					...
					try {
						Set<String> params=Core.getInputParameters(this.microflowName).keySet();

						MicroflowCallBuilder builder = Core.microflowCall(this.microflowName);
						if (!params.isEmpty()) {
							String paramName = params.iterator().next();
							builder = builder.withParam(paramName, this.action);
						}
						Object booleanResult = builder.execute(this.context);
					...
					for( IMendixObject followUpAction : followUpResult )
					{
						...
						if( processId != null ) { 
							...
							IContext followupContext = followUpResult.size() > 1 ? this.context.getSession().createContext() : this.context;
							QueueHandler.getQueueHandler().appendActionForProcessing(followupContext, followUpAction, processObj, true);
						}
						else {
	...
	private static void setErrormessageAndCommit(IContext context, IMendixObject queuedAction, String error, Exception stacktrace, LogExecutionStatus status, ActionStatus phase) 
	{
		try 
		{
			...
			Core.microflowCall("ProcessQueue.SF_WriteExecutionLog").withParams(paramMap).execute(context);
		} catch (Exception e) {
	...
	private void setExecutionLog(LogExecutionStatus status, ActionStatus phase)
	{
		try 
		{
			...
			Core.microflowCall("ProcessQueue.SF_WriteExecutionLog").withParams(paramMap).execute(this.context);
		} catch (Exception e) {
