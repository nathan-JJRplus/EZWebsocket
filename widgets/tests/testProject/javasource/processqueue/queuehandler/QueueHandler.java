...
//		this.running = false;
	...
	public void initializeQueue( IContext context, IMendixObject queueConfiguration ) throws CoreException {
		...
		_node.info("Start or update queue: " + queueNr  +  " - " + queueName);
		_node.trace("Start or update queue: " + queueNr +  " - " + queueName + " with "+ nrOfThreads + " threads.");
		...
		if (existingTpool != null) {
			if(nrOfThreads > existingTpool.getCorePoolSize()) {
				existingTpool.setMaximumPoolSize(nrOfThreads);
				existingTpool.setCorePoolSize(nrOfThreads);
			} else {
				existingTpool.setCorePoolSize(nrOfThreads);
				existingTpool.setMaximumPoolSize(nrOfThreads);
			}
			existingTpool.setKeepAliveTime(Long.MAX_VALUE, TimeUnit.MILLISECONDS);
			existingTpool.setThreadFactory(customThreadFactory);

			queueMap.put(queueNr, existingTpool); // replace existing tPool
		} else {
